...
void setup_executable_buffer() {
	...
	jmp_buffer = (uint8_t *) mmap(jmp_buffer, JMP_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
void setup_registers() {
	...
	return_point = (uintptr_t) exec_buffer;
	register_save.rsp = ((uintptr_t) stack_buffer) + STACK_SIZE;
	setup_cpu();
}
...
uint64_t asm_execute(void * buffer, ssize_t size, int skip) {
	...
	exec_buffer[exec_buffer_size + 0] = 0xff; // jmp [rel $ + 6]
	...
	*(uintptr_t *) &exec_buffer[exec_buffer_size + 6] = (uintptr_t) asm_exit_context;
...
void * asm_append_jmptable(void * symbol) {
	...
	jmp_buffer[jmp_buffer_size + 0] = 0xff; // jmp [rel $ + 6]
	jmp_buffer[jmp_buffer_size + 1] = 0x25;
	jmp_buffer[jmp_buffer_size + 2] = 0x00;
	jmp_buffer[jmp_buffer_size + 3] = 0x00;
	jmp_buffer[jmp_buffer_size + 4] = 0x00;
	jmp_buffer[jmp_buffer_size + 5] = 0x00;
	*(uintptr_t *) &jmp_buffer[jmp_buffer_size + 6] = (uintptr_t) symbol;
